...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setActiveTimers((prev) => {
        const updated: ActiveTimerState = {} as ActiveTimerState;
  ...
  const saveProjects = async (updatedProjects: Project[]) => {
    await storageSet('dev-projects-v2', JSON.stringify(updatedProjects));
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        {showNewProject && (
          <NewProjectForm onAdd={addProject} onCancel={() => setShowNewProject(false)} />
        )}
...
interface NewProjectFormProps {
  onAdd: (name: string, priority: ProjectPriority, startDate?: string, endDate?: string) => void;
  onCancel: () => void;
}
...
const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  const handleSaveTask = () => {
    ...
    if (editTaskText.trim()) {
      const updatedTasks = project.tasks.map((t) => (t.id === editingTask ? { ...t, text: editTaskText.trim() } : t));
      onUpdate({ tasks: updatedTasks });
      setEditingTask(null);
    }
  };

  const handleEditTaskKeyPress = (e: React.KeyboardEvent<HTMLInputElement>) => {
            ...
            {project.startDate && project.endDate && (
              <div className="mt-3">
                <div className="flex justify-between text-sm text-gray-600 mb-1">
                  <span>Progresso</span>
                  <span>
                    {Math.round(((project.tasks.filter((t) => t.completed).length || 0) / (project.tasks.length || 1)) * 100) || 0}%
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    ...
                    style={{
                      width: `${Math.round(((project.tasks.filter((t) => t.completed).length || 0) / (project.tasks.length || 1)) * 100) || 0}%`,
                    }}
                  />
                </div>
              </div>
          ...
          <div className="mt-6">
            <div className="flex gap-2 border-b border-gray-200 mb-4">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 font-medium transition-colors ${
                  activeTab === 'notes' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600 hover:text-gray-900'
                }`}
              >
                <FileText size={18} />
                Notas
              ...
              <div className="max-h-[480px] overflow-auto">
                ...
                <div className="space-y-2">
                  {project.tasks.length === 0 ? (
                    <p className="text-gray-500 text-center py-8">Nenhuma tarefa ainda. Adicione uma para começar!</p>
                  ) : (
